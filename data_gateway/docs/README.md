# 数据网关服务 - 架构设计与功能说明

## 目录

- [服务概述](#服务概述)
- [架构设计](#架构设计)
- [数据源说明](#数据源说明)
- [功能说明](#功能说明)
- [API 接口](#api-接口)
- [数据库设计](#数据库设计)
- [快速开始](#快速开始)
- [配置说明](#配置说明)
- [开发指南](#开发指南)
- [故障排查](#故障排查)

---

## 服务概述

数据网关服务是一个独立的数据接口服务，为外部平台提供统一的金融数据获取接口。

### 核心特性

- **多市场支持**: A股、港股、美股、期货、经济指标
- **多数据源自动切换**: AKShare (实时) + BaoStock (历史) + Miana (商业数据)
- **多种接口**: HTTP API + Redis 消息队列
- **Docker 部署**: 开箱即用
- **统一数据格式**: 标准化返回
- **定时同步**: 支持批量历史数据同步
- **健康监控**: 数据源健康状态实时监控

### 服务端口

| 服务 | 端口 |
|------|------|
| 数据网关 API | 8001 |
| PostgreSQL | 5432 |
| Redis | 6379 |

---

## 架构设计

### 系统架构

```
┌─────────────────────────────────────────────────────────────────┐
│                        客户端应用                              │
│                     (SAPAS / 第三方平台)                       │
└───────────────────────┬─────────────────────────────────────────┘
                        │ HTTP API
                        ▼
┌─────────────────────────────────────────────────────────────────┐
│                    数据网关服务 (FastAPI)                      │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │           API 路由层 (routes.py)                        │  │
│  │  - /api/v1/quote      实时行情                          │  │
│  │  - /api/v1/kline      K线数据                           │  │
│  │  - /api/v1/fundamentals 基本面数据                      │  │
│  │  - /api/v1/money-flow 资金流向 (Miana)                  │  │
│  │  - /api/v1/sectors/*   板块数据 (Miana)                │  │
│  └─────────────────────────────────────────────────────────┘  │
│                          │                                     │
│  ┌───────────────────────▼────────────────────────────────┐  │
│  │         网关管理器 (gateway/manager.py)                  │  │
│  │         - 路由到对应市场网关                            │  │
│  │         - 数据源自动切换                                │  │
│  └───────────────────────┬────────────────────────────────┘  │
│                         │                                      │
│  ┌──────────────────────┼───────────────────────────────────┐  │
│  │                      │                                   │  │
│  ▼                      ▼                                   ▼  │
│ ┌─────────┐      ┌─────────┐      ┌─────────────┐             │
│ │ A股网关 │      │ 港股网关 │ ...  │ 经济指标网关 │             │
│ └────┬────┘      └────┬────┘      └──────┬──────┘             │
└──────┼───────────────┼────────────────┼──────────────────────┘
       │               │                │
       ▼               ▼                ▼
┌─────────────────────────────────────────────────────────────────┐
│                      数据源层                                   │
│  ┌──────────┐  ┌──────────┐  ┌─────────────┐                  │
│  │ AKShare  │  │ BaoStock │  │  Miana平台  │                  │
│  │ (实时)   │  │ (历史)   │  │  (商业数据) │                  │
│  └────┬─────┘  └────┬─────┘  └──────┬──────┘                  │
└───────┼────────────┼────────────────┼──────────────────────────┘
        │            │                │
        └────────────┴────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────────┐
│                     PostgreSQL 数据库                           │
│  - request_logs    API 请求日志                                │
│  - stock_daily_k   每日K线数据                                 │
│  - sync_logs       数据同步日志                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 目录结构

```
data_gateway/
├── src/                         # 源代码
│   ├── main.py                 # FastAPI 应用入口
│   ├── config.py               # 配置管理
│   ├── database.py             # 数据库连接
│   ├── gateway/               # 数据网关
│   │   ├── base.py           # 基类 (DataSource, MarketGateway)
│   │   ├── manager.py        # 网关管理器
│   │   ├── markets/          # 市场实现
│   │   │   ├── cn_a.py       # A股市场
│   │   │   ├── hk.py         # 港股市场
│   │   │   ├── us.py         # 美股市场
│   │   │   ├── futures.py    # 期货市场
│   │   │   └── economic.py   # 经济指标
│   │   └── sources/          # 数据源实现
│   │       └── miana_source.py  # Miana数据源
│   ├── api/                  # API 路由
│   │   ├── routes.py         # 公共API
│   │   └── admin_routes.py   # 管理API
│   ├── services/             # 业务服务
│   │   ├── sync_service.py   # 数据同步服务
│   │   └── scheduler_service.py  # 定时调度
│   ├── models/               # 数据模型
│   │   ├── kline.py
│   │   ├── stock_daily_k.py
│   │   └── sync_log.py
│   └── utils/               # 工具
│       └── logger.py
├── scripts/                 # 脚本
│   ├── init_db.py          # 数据库初始化
│   └── sync_all.py         # 数据同步脚本
├── logs/                   # 日志目录
├── docs/                   # 文档目录
├── Dockerfile              # Docker 构建文件
├── requirements.txt        # Python 依赖
└── README.md              # 本文件
```

---

## 数据源说明

### 数据源对比

| 数据源 | 用途 | 实时数据 | 历史数据 | 特色功能 |
|--------|------|---------|---------|---------|
| **AKShare** | 开源免费数据 | ✅ | ✅ | 覆盖面广，更新及时 |
| **BaoStock** | 证券数据 | ⚠️ 延迟 | ✅ | 财务数据全面 |
| **Miana** | 商业数据平台 | ✅ | ✅ | 资金流向、板块数据、五档盘口 |

### Miana 数据平台

Miana 是商业数据平台，提供高质量的专业数据：

**官网**: https://miana.com.cn

**数据特点**:
- 全球覆盖：30+ 国家，40+ 交易所
- 实时数据：包含五档盘口数据
- 资金流向：当日资金流向、大单分类
- 历史数据：30+ 年
- 股票数量：100,000+

**特色数据**:
1. **实时五档盘口**: 买一~买五、卖一~卖五的价格和量
2. **资金流向**: 大单、中单、小单净流入分类
3. **板块数据**: 行业板块、概念板块实时行情
4. **市场数据**: 市值、股本、涨跌停价格等

**配置方式**:
在环境变量中设置 `MIANA_TOKEN` 即可启用。

---

## 功能说明

### 支持的市场

| 市场 | 代码 | 实时行情 | K线 | 基本面 | 资金流向 | 板块数据 |
|------|------|---------|-----|--------|---------|---------|
| A股 | `cn_a` | ✅ AKShare/Miana | ✅ | ✅ | ✅ Miana | ✅ Miana |
| 港股 | `hk` | ✅ AKShare | ✅ | ❌ | ❌ | ❌ |
| 美股 | `us` | ✅ AKShare | ✅ | ❌ | ❌ | ❌ |
| 期货 | `futures` | ✅ AKShare | ✅ | ❌ | ❌ | ❌ |
| 经济指标 | `economic` | ❌ | ✅ | ❌ | ❌ | ❌ |

### 数据同步功能

**支持的数据同步**:
- 股票列表同步
- 每日K线数据同步
- 增量更新（只同步最新交易日数据）

**调度服务**:
- 自动检测交易日
- 定时执行数据更新
- 同步任务状态跟踪

---

## API 接口

### 公共 API

#### 1. 获取实时行情

```bash
POST /api/v1/quote
Content-Type: application/json

{
  "market": "cn_a",
  "symbols": ["000001", "600000"]
}
```

**响应**:
```json
{
  "code": 0,
  "message": "success",
  "data": {
    "000001": {
      "symbol": "000001",
      "name": "平安银行",
      "price": 11.23,
      "open": 11.20,
      "high": 11.30,
      "low": 11.15,
      "volume": 12345678,
      "amount": 138765432.1,
      "change": 0.05,
      "change_pct": 0.45,
      "bid": 11.22,
      "ask": 11.24,
      "timestamp": "2026-01-26 10:30:00",
      "market": "cn_a"
    }
  }
}
```

#### 2. 获取K线数据

```bash
GET /api/v1/kline?market=cn_a&symbol=000001&period=daily&start_date=2025-01-01&end_date=2026-01-26
```

**参数说明**:
- `market`: 市场代码 (cn_a, hk, us, futures, economic)
- `symbol`: 股票代码
- `period`: 周期 (1m, 5m, 15m, 30m, 60m, daily, weekly, monthly)
- `start_date`: 开始日期 YYYY-MM-DD
- `end_date`: 结束日期 YYYY-MM-DD

**响应**:
```json
{
  "code": 0,
  "message": "success",
  "data": [
    {
      "symbol": "000001",
      "datetime": "2026-01-26",
      "open": 11.20,
      "close": 11.23,
      "high": 11.30,
      "low": 11.15,
      "volume": 12345678,
      "amount": 138765432.1,
      "period": "daily",
      "market": "cn_a"
    }
  ]
}
```

#### 3. 获取基本面数据

```bash
GET /api/v1/fundamentals?market=cn_a&symbol=000001
```

**响应**:
```json
{
  "code": 0,
  "message": "success",
  "data": {
    "symbol": "000001",
    "name": "平安银行",
    "market": "cn_a",
    "revenue": 158763000000,
    "net_profit": 45632000000,
    "eps": 2.35,
    "bvps": 18.45,
    "pe": 4.78,
    "pb": 0.61,
    "market_cap": 217654321000
  }
}
```

#### 4. 健康检查

```bash
GET /health
```

**响应**:
```json
{
  "service": "data-gateway",
  "status": "healthy",
  "timestamp": "2026-01-26T10:30:00",
  "markets": {
    "cn_a": true,
    "hk": true,
    "us": true,
    "futures": true,
    "economic": true
  }
}
```

### 缅A平台特色 API

#### 5. 获取资金流向

```bash
GET /api/v1/money-flow/600519?date=2026-01-26
```

**参数说明**:
- `symbol`: 股票代码
- `date`: 日期 YYYY-MM-DD，默认当天

**响应**:
```json
{
  "code": 0,
  "message": "success",
  "data": {
    "symbol": "600519",
    "date": "2026-01-26",
    "large_inflow": 1234567890,
    "large_outflow": 987654321,
    "medium_inflow": 234567890,
    "medium_outflow": 123456789,
    "small_inflow": 34567890,
    "small_outflow": 12345678,
    "net_inflow": 123456789
  }
}
```

#### 6. 获取行业板块

```bash
GET /api/v1/sectors/industry
```

**响应**:
```json
{
  "code": 0,
  "message": "success",
  "data": [
    {
      "sector_name": "银行",
      "change_pct": 1.23,
      "amount": 1234567890,
      "stock_count": 42
    }
  ]
}
```

#### 7. 获取概念板块

```bash
GET /api/v1/sectors/concept
```

**响应**:
```json
{
  "code": 0,
  "message": "success",
  "data": [
    {
      "sector_name": "人工智能",
      "change_pct": 2.45,
      "amount": 987654321,
      "stock_count": 56
    }
  ]
}
```

### 管理 API

#### 创建同步任务

```bash
POST /api/v1/admin/sync/create
```

#### 查询同步状态

```bash
GET /api/v1/admin/sync/status/{task_id}
```

#### 获取活跃同步任务

```bash
GET /api/v1/admin/sync/active
```

#### 快速同步

```bash
POST /api/v1/admin/sync/quick
```

#### 获取股票列表

```bash
GET /api/v1/admin/stocks/list
```

#### 获取统计信息

```bash
GET /api/v1/admin/stats
```

---

## 数据库设计

### 数据库概述

数据网关服务使用独立的 `data_gateway` 数据库，与 SAPAS 主系统的 `sapas_db` 完全分离。

### 数据库对比

| 数据库 | 用途 | 主要表 |
|--------|------|--------|
| `sapas_db` | SAPAS 主系统 | stocks, users, backtests, alerts |
| `data_gateway` | 数据网关服务 | stock_daily_k, sync_logs, request_logs |

### 核心表结构

#### 1. stock_daily_k - 每日K线数据

```sql
CREATE TABLE stock_daily_k (
    id SERIAL PRIMARY KEY,
    ts_code VARCHAR(20) NOT NULL,      -- 股票代码
    trade_date DATE NOT NULL,           -- 交易日期
    open DECIMAL(10, 2),                -- 开盘价
    high DECIMAL(10, 2),                -- 最高价
    low DECIMAL(10, 2),                 -- 最低价
    close DECIMAL(10, 2),               -- 收盘价
    pre_close DECIMAL(10, 2),           -- 前收盘价
    vol DECIMAL(15, 2),                 -- 成交量(手)
    amount DECIMAL(15, 2),              -- 成交额(千元)
    adj_factor DECIMAL(10, 6),          -- 复权因子
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(ts_code, trade_date)
);
```

**索引**:
```sql
CREATE INDEX idx_stock_daily_k_ts_code ON stock_daily_k(ts_code);
CREATE INDEX idx_stock_daily_k_trade_date ON stock_daily_k(trade_date);
```

#### 2. sync_logs - 同步日志

```sql
CREATE TABLE sync_logs (
    id SERIAL PRIMARY KEY,
    sync_type VARCHAR(50) NOT NULL,     -- 同步类型: stock_list, daily_k
    status VARCHAR(20) NOT NULL,         -- 状态: running, completed, failed
    start_time TIMESTAMP NOT NULL,
    end_time TIMESTAMP,
    records_processed INTEGER DEFAULT 0,
    error_message TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

#### 3. request_logs - API 请求日志

```sql
CREATE TABLE request_logs (
    id SERIAL PRIMARY KEY,
    endpoint VARCHAR(100) NOT NULL,
    method VARCHAR(10) NOT NULL,
    request_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    response_time INTEGER,               -- 响应时间(毫秒)
    status_code INTEGER,
    ip_address VARCHAR(50)
);
```

### 维护命令

```sql
-- 查看今日K线数据
SELECT ts_code, trade_date, close, volume
FROM stock_daily_k
WHERE trade_date = CURRENT_DATE
ORDER BY volume DESC
LIMIT 10;

-- 查看同步记录
SELECT * FROM sync_logs
ORDER BY start_time DESC
LIMIT 10;

-- 清理旧日志（保留30天）
DELETE FROM request_logs
WHERE request_time < CURRENT_DATE - INTERVAL '30 days';
```

---

## 快速开始

### 方式一: Docker 部署 (推荐)

```bash
# 1. 进入项目目录
cd data_gateway

# 2. 启动服务
docker-compose up -d

# 3. 查看日志
docker-compose logs -f data-gateway

# 4. 访问文档
open http://localhost:8001/docs
```

### 方式二: 本地运行

#### Linux/Mac

```bash
# 1. 安装依赖
pip install -r requirements.txt

# 2. 配置环境变量
cp .env.example .env
# 编辑 .env 文件

# 3. 初始化数据库
python scripts/init_db.py

# 4. 启动服务
python run_server.py
```

#### Windows

```cmd
# 1. 安装依赖
pip install -r requirements.txt

# 2. 配置环境变量
copy .env.example .env
REM 编辑 .env 文件

# 3. 初始化数据库
python scripts\init_db.py

# 4. 启动服务
python run_server.py
```

---

## 配置说明

### 环境变量

创建 `.env` 文件并配置以下参数：

```bash
# ============== 服务配置 ==============
DG_HOST=0.0.0.0
DG_PORT=8001
DG_VERSION=1.0.0
DG_DEBUG=false
DG_LOG_LEVEL=INFO

# ============== 数据库配置 ==============
DG_DATABASE_URL=postgresql+asyncpg://user:password@localhost:5432/data_gateway

# ============== Redis 配置 ==============
DG_REDIS_URL=redis://localhost:6379/0

# ============== Miana 数据平台配置 ==============
MIANA_TOKEN=your_miana_token_here

# ============== 日志配置 ==============
DG_LOG_FILE=logs/data_gateway.log
DG_LOG_ROTATION=10 MB
DG_LOG_RETENTION=30 days
```

---

## 开发指南

### 添加新市场

1. 在 `src/gateway/markets/` 创建新文件
2. 继承 `MarketGateway` 类
3. 实现数据源 `DataSource`
4. 在 `manager.py` 注册新市场

示例：
```python
from ..base import MarketGateway

class NewMarketGateway(MarketGateway):
    def __init__(self):
        super().__init__("new_market")
        self.sources = [...]
```

### 添加新数据源

1. 继承 `DataSource` 基类
2. 实现三个方法：
   - `get_quote()`
   - `get_kline()`
   - `get_fundamentals()`

示例：
```python
from ..base import DataSource

class NewDataSource(DataSource):
    def __init__(self):
        super().__init__("NewSource")

    async def get_quote(self, symbols, market):
        # 实现获取实时行情
        pass

    async def get_kline(self, symbol, period, start_date, end_date, market):
        # 实现获取K线数据
        pass

    async def get_fundamentals(self, symbol, market):
        # 实现获取基本面数据
        pass
```

---

## 故障排查

### 服务启动失败

```bash
# 检查端口占用
netstat -tulpn | grep 8001

# 查看日志
tail -f logs/data_gateway.log
```

### 数据库连接失败

```bash
# 检查 PostgreSQL 状态
docker-compose ps postgres

# 测试连接
psql -h localhost -U postgres -d data_gateway
```

### 数据获取失败

```bash
# 检查数据源健康
curl http://localhost:8001/health

# 查看同步日志
curl http://localhost:8001/api/v1/admin/sync/active
```

### Docker 构建失败

Windows 下可能出现文件权限问题，请确保：
1. 删除 `__pycache__` 目录
2. 删除 `venv` 虚拟环境目录
3. 不包含符号链接文件

---

## 功能差距分析

### 当前支持的功能

| 功能模块 | 支持情况 | 数据源 |
|---------|---------|--------|
| 实时行情 | ✅ 完整支持 | AKShare + Miana |
| K线数据 | ✅ 完整支持 | AKShare + BaoStock + Miana |
| 基本面数据 | ✅ 完整支持 | BaoStock |
| 资金流向 | ✅ 完整支持 | Miana |
| 板块数据 | ✅ 完整支持 | Miana |
| 五档盘口 | ✅ 完整支持 | Miana |
| 数据同步 | ✅ 完整支持 | BaoStock |
| 健康监控 | ✅ 完整支持 | 内置 |

### 待开发功能

| 功能 | 优先级 | 说明 |
|------|-------|------|
| 分钟K线 | 🔴 高 | 扩展现有K线接口支持分钟级 |
| 龙虎榜 | 🟡 中 | 需要添加AKShare接口 |
| WebSocket推送 | 🟡 中 | 实时数据推送 |
| ETF数据 | 🟢 低 | 建议使用AKShare |
| 大单交易 | 🟢 低 | 详细的逐笔大单数据 |

---

## 许可证

MIT License
